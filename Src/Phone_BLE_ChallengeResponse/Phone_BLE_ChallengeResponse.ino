...
bool connectToVehicle() {
  ...
  String challengeValue = pChallengeCharacteristic->readValue();
  
  if (challengeValue.length() == 16) {
    Serial.println("✓ Challenge read successfully!");
    printHex("Challenge: ", (const uint8_t*)challengeValue.c_str(), 16);
    
    // Compute and send response
    uint8_t response[32];
    bool success = computeHMAC(pairingKey, 16, (const uint8_t*)challengeValue.c_str(), 16, response);
    ...
    if (success) {
      ...
      String authResult = pAuthCharacteristic->readValue();
